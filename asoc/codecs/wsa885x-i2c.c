...
void reinit_wsa885x_powerup(struct wsa885x_i2c_priv *wsa885x)
{
	...
	regmap_write(wsa885x->regmap,
			SMP_AMP_CTRL_STEREO_FU21_CH_VOL_CH2X0_MSB, wsa885x->stereo_voldB);
	regmap_write(wsa885x->regmap, SMP_AMP_CTRL_STEREO_FU21_CH_VOL_CH2X0_LSB, 0x00);
	regmap_write(wsa885x->regmap,
			SMP_AMP_CTRL_STEREO_FU21_CH_VOL_CH2X1_MSB, wsa885x->stereo_voldB);
	regmap_write(wsa885x->regmap, SMP_AMP_CTRL_STEREO_FU21_CH_VOL_CH2X1_LSB, 0x00);
...
static void wsa885x_temperature_work(struct work_struct *work)
{
	...
	do {
		...
		if (temp_val <= LOW_TEMP_THRESHOLD ||
			...
			if (retry--)
				usleep_range(10000, 11000);  /* 10-11ms */
		} else {
			break;
		}
	} while (retry);
