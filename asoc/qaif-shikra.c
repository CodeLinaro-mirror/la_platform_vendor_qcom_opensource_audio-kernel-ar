...
static int shikra_qaif_get_dma_idx(unsigned int dai_id)
{
	...
	case LPASS_CDC_DMA_VA_TX0 ... LPASS_CDC_DMA_VA_TX8:
		...
		break;
	default:
		pr_err("DAI ID not Supported\n");
		break;
	}

	pr_err("DAI ID %u not found in map\n", dai_id);
	return -EINVAL;
}
					  ...
					  int direction, unsigned int dai_id)
{
	...
	case MI2S_PRIMARY ... MI2S_QUINARY:
		if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			index = v->wrdma_start + shikra_qaif_get_dma_idx(dai_id);
	...
	case LPASS_CDC_DMA_VA_TX0 ... LPASS_CDC_DMA_VA_TX8:
		if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			index = v->codec_wrdma_start + shikra_qaif_get_dma_idx(dai_id);
...
static struct qaif_variant shikra_qaif_data = {
	...
	.cif_rddma_en_16bit_unpack		= REG_FIELD_ID(0xC05C, 0, 0, 4, 0x1000),    // ENABLE_16B_UNPACKING [0]
	...
	.cif_rddma_active_ch_en		= REG_FIELD_ID(0xC05C, 12, 27, 4, 0x1000),     // ACTIVE_CHANNEL_EN [27:12]
	...
	.cif_wrdma_en_16bit_unpack		= REG_FIELD_ID(0x15058, 0, 0, 4, 0x1000),    // ENABLE_16B_PACKING [0]
	...
	.cif_wrdma_active_ch_en			= REG_FIELD_ID(0x15058, 12, 27, 4, 0x1000),     // ACTIVE_CHANNEL_EN [27:12]

	/* AUDIO_CORE_QAIF_AUD_INTFa_SYNC_CFG (0x4004 + 0x1000*a) */
	.aif_inv_sync					= REG_FIELD_ID(0x4004, 12, 12, 4, 0x1000),  /* bit 12 */
	.aif_sync_delay					= REG_FIELD_ID(0x4004, 9, 8, 4, 0x1000),    /* bits 9:8 */
	.aif_sync_mode					= REG_FIELD_ID(0x4004, 5, 4, 4, 0x1000),    /* bits 5:4 */
	.aif_sync_src					= REG_FIELD_ID(0x4004, 0, 0, 4, 0x1000),    /* bit 0 */

	/* AUDIO_CORE_QAIF_AUD_INTFa_BIT_WIDTH_CFG (0x4008 + 0x1000*a) */
	.aif_sample_width_rx			= REG_FIELD_ID(0x4008, 28, 24, 4, 0x1000), /* bits 28:24 */
	.aif_sample_width_tx			= REG_FIELD_ID(0x4008, 20, 16, 4, 0x1000), /* bits 20:16 */
	.aif_slot_width_rx				= REG_FIELD_ID(0x4008, 12, 8, 4, 0x1000),  /* bits 12:8 */
	.aif_slot_width_tx				= REG_FIELD_ID(0x4008, 4, 0, 4, 0x1000),   /* bits 4:0 */

	/* AUDIO_CORE_QAIF_AUD_INTFa_FRAME_CFG (0x400C + 0x1000*a) */
	.aif_bits_per_lane				= REG_FIELD_ID(0x400C, 9, 0, 4, 0x1000),   /* bits 9:0 */

	/* AUDIO_CORE_QAIF_AUD_INTFa_ACTV_SLOT_EN_TX (0x4010 + 0x1000*a) */
	.aif_slot_en_tx_mask			= REG_FIELD_ID(0x4010, 31, 0, 4, 0x1000),  /* bits 31:0 */

	/* AUDIO_CORE_QAIF_AUD_INTFa_ACTV_SLOT_EN_RX (0x4030 + 0x1000*a) */
	.aif_slot_en_rx_mask			= REG_FIELD_ID(0x4030, 31, 0, 4, 0x1000),  /* bits 31:0 */
	...
	.aif_lane_en					= REG_FIELD_ID(0x4050, 15, 8, 4, 0x1000),  /* bits 15:8 */
	.aif_lane_dir					= REG_FIELD_ID(0x4050, 7, 0, 4, 0x1000),   /* bits 7:0 */
...
static struct platform_driver shikra_qaif_cpu_platform_driver = {
	...
	.remove_new = asoc_qcom_qaif_cpu_platform_remove,
