...
#define QAIF_IRQ_DMA_ALL				(0xf)	//TODO: confirm if only dma0...3 are active.
 ...
 * for example, Display port can be part of the below list in future*/
...
} qaif_summary_irq_bitmask_t;

// defines the bitmask in the status register for each of the clients
typedef enum qaif_client_status_register_bitmask_info
{
...
} qaif_client_bitmask_status_register_info_t;

struct qaif_irq_map {
	int client_id;
	u32 mask;
	irqreturn_t (*client_irq_handler)(struct qaif_drv_data *, u32 );
};
