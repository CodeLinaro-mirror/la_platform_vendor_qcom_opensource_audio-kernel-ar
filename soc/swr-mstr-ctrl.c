...
static void swrm_copy_data_port_config(struct swr_master *master, u8 bank)
{
	...
	/* ch_mask and offset1 configuration for slave port */
	u8 agg_slv_port_ch_mask[SWR_MAX_DEV_NUM][SWR_MAX_DEV_PORT_NUM];
	u8 agg_slv_port_offset1[SWR_MAX_DEV_NUM][SWR_MAX_DEV_PORT_NUM];
	...
	for (i = 0; i < SWR_MSTR_PORT_LEN; i++) {
		...
		list_for_each_entry(port_req, &mport->port_req_list, list) {
			...
			j++;
			slv_port_id = port_req->slave_port_id;
			...
			agg_slv_port_ch_mask[port_req->dev_num][slv_port_id] |=
							port_req->req_ch;

			if (len < SWRM_MAX_PORT_REG) {
				reg[len] = REGISTER_ADDRESS(swrm->version_index,
						SWRM_CMD_FIFO_WR_CMD);
				val[len++] = SWR_REG_VAL_PACK(agg_slv_port_ch_mask[port_req->dev_num][slv_port_id],
						port_req->dev_num, get_cmd_id(swrm),
						SWRS_DP_CHANNEL_ENABLE_BANK(slv_port_id,
									bank));
			}
			...
			if (len < SWRM_MAX_PORT_REG) {
				...
				val[len++] = SWR_REG_VAL_PACK(
						...
						port_req->dev_num, get_cmd_id(swrm),
						SWRS_DP_SAMPLE_CONTROL_1_BANK(slv_port_id,
									bank));
			}
			...
			if (agg_slv_port_offset1[port_req->dev_num][slv_port_id] == 0)
				agg_slv_port_offset1[port_req->dev_num][slv_port_id] =
						port_req->offset1;
			else {
				agg_slv_port_offset1[port_req->dev_num][slv_port_id] =
					min(agg_slv_port_offset1[port_req->dev_num][slv_port_id],
							port_req->offset1);
			}
			if (len < SWRM_MAX_PORT_REG
				...
				reg[len] = REGISTER_ADDRESS(swrm->version_index,
						SWRM_CMD_FIFO_WR_CMD);
				val[len++] =
				SWR_REG_VAL_PACK(agg_slv_port_offset1[port_req->dev_num][slv_port_id],
						port_req->dev_num, get_cmd_id(swrm),
						SWRS_DP_OFFSET_CONTROL_1_BANK(slv_port_id,
									bank));
			}
			...
			if (len < SWRM_MAX_PORT_REG
				...
				val[len++] = SWR_REG_VAL_PACK(hparams,
						port_req->dev_num, get_cmd_id(swrm),
						SWRS_DP_HCONTROL_BANK(slv_port_id,
									bank));
			}
			...
			if (len < SWRM_MAX_PORT_REG
				...
				val[len++] =
					SWR_REG_VAL_PACK(port_req->word_length,
						port_req->dev_num, get_cmd_id(swrm),
						SWRS_DP_BLOCK_CONTROL_1(slv_port_id));
			}
			...
			if (len < SWRM_MAX_PORT_REG
				...
				val[len++] =
					...
					port_req->dev_num, get_cmd_id(swrm),
					SWRS_DP_BLOCK_CONTROL_3_BANK(slv_port_id,
									bank));
			}
			...
			if (len < SWRM_MAX_PORT_REG
				...
				if (len < SWRM_MAX_PORT_REG) {
					...
					val[len++] = SWR_REG_VAL_PACK(0, port_req->dev_num,
								get_cmd_id(swrm),
								SWRS_DP_PORT_CONTROL(slv_port_id));
				}
...
static void swrm_update_wordlength(struct swr_mstr_ctrl *swrm)
{
	u8 i = 0;
...
static int swrm_master_init(struct swr_mstr_ctrl *swrm)
{
	...
	u8 ssp_period = 1;
	u8 retry_cmd_num = 3, retry_reset_chk = 10;
	...
	if (swrm->version <= SWRM_VERSION_4_0 &&
	...
	if (!swrm_check_link_status(swrm, 0x1)) {
		dev_err(swrm->dev, "%s: swr link failed to connect\n", __func__);
		for (i = 0; i < len; i++) {
			usleep_range(50, 55);
			dev_err(swrm->dev, "%s:reg:0x%x val:0x%x\n", __func__,
	...
	if (swrm->version <= SWRM_VERSION_4_0 &&
		...
		while (swr_master_read(swrm, SWRM_COMP_STATUS) & 0x02) {
			usleep_range(50, 55);
			if (--retry_reset_chk == 0)
				break;
		}
		dev_dbg(swrm->dev, "%s: BUS reset check completed 0x%X:\n",
			__func__, swr_master_read(swrm, SWRM_COMP_STATUS));
		...
		if (!swrm_check_link_status(swrm, 0x0))
			dev_err(swrm->dev, "%s:failed in disconnecting?\n",
					__func__);

		/* Update word length for all ports. */
		swrm_update_wordlength(swrm);

		/* Enable auto enumeration. */
		swr_master_write(swrm, SWRM_ENUMERATOR_CFG, 0x1);
		swr_master_write(swrm, SWRM_COMP_FEATURE_CFG, 0x40E);

		/* Enable interrupts and clk. */
		swrm->intr_mask = REGISTER_ADDRESS(swrm->version_index,
						SWRM_INTERRUPT_STATUS_MASK);
		swr_master_write(swrm,
			REGISTER_ADDRESS(swrm->version_index, SWRM_INTERRUPT_EN), swrm->intr_mask);
		swr_master_write(swrm, SWRM_CLK_CTRL(swrm->ee_val), 0x1);
		...
		if (!swrm_check_link_status(swrm, 0x1)) {
			...
			for (i = 0; i < len; i++) {
				usleep_range(50, 55);
				dev_err(swrm->dev, "%s:reg:0x%x val:0x%x\n",
					__func__, reg[i], swr_master_read(swrm, reg[i]));
			}
			return -EINVAL;
		}
	}
...
static int swrm_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(pdev->dev.of_node, "qcom,swr-port-wordlength",
				&map_size)) {
		dev_info(swrm->dev, "word length given\n");

		map_length = map_size / sizeof(u32);
		...
		ret = of_property_read_u32_array(pdev->dev.of_node,
				"qcom,swr-port-wordlength", temp, map_length);
		if (ret) {
			dev_err(swrm->dev, "%s: Failed to read port mapping\n",
						__func__);
...
static int swrm_runtime_resume(struct device *dev)
{
	...
	struct swr_device *swr_dev;
	struct irq_data *irqd;
	u32 temp = 0, val = 0, swr_rst_chk_cnt = 5;
	...
	if ((swrm->state == SWR_MSTR_DOWN) ||
		...
		if (!swrm->clk_stop_mode0_supp || swrm->state == SWR_MSTR_SSR) {
			...
			if (swrm_first_after_clk_enabled(swrm) || swrm->mstr_init_required) {
				...
				while (swr_rst_chk_cnt--) {
					val = swr_master_read(swrm, SWRM_COMP_SW_RESET);
					dev_dbg(dev, "%s Read SWRM_COMP_SW_RESET value 0x%x\n",
						__func__, val);
					if (val == 0x0)
						break;
					usleep_range(100, 105);
